...
from van import Attention, Mlp, Block
...
class MHCABlock(nn.Module):
    ...
    def __init__(
        ...
        self.blk = Block(dim, mlp_ratio=mlp_ratio,)
...
class MPViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # print("after_stem,shape", x.shape)
        for idx in range(self.num_stages):
            att_inputs = self.patch_embed_stages[idx](x)
            # print("attn_inputs")
            # for att in att_inputs:
                # print(att.shape)
            # print("mhca")
...
if __name__ == "__main__":
    model = van_mpvit_xsmall(num_classes=10,)
    model.eval()
